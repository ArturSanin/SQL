/*
    ...
    property to all user-defined stored procedures in the specified database that do not have a
    description yet.
    If all user-defined stored procedures already have an MS_Description extended property, NULL is returned.
...
SELECT
	...
	STRING_AGG(
		N'-- =============================================' + CHAR(13) + CHAR(10) +
		N'-- MS_Description for ' + N'stored procedure ' + SCHEMA_NAME([p].[schema_id]) + N'.' + CAST([p].[name] AS nvarchar(MAX)) + CHAR(13) + CHAR(10) + 
		N'-- =============================================' + CHAR(13) + CHAR(10) +
		...
		N'@level1type = N''PROCEDURE'', @level1name = N''' + [p].[name] + N''';' + CHAR(13) + CHAR(10) + 
		N'GO',
		CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10)
	) WITHIN GROUP (ORDER BY SCHEMA_NAME([p].[schema_id]) ASC, [p].[name] ASC) AS [Script]
FROM
	[sys].[procedures] [p]
LEFT JOIN
	[sys].[extended_properties] [ep] ON [p].[object_id] = [ep].[major_id] AND [ep].[name] = N'MS_Description' AND [ep].[minor_id] = 0
WHERE
	[p].[is_ms_shipped] = 0  -- Excluding system stored procedures.
AND
	[ep].[value] IS NULL;  -- Excluding stored procedures that already have a description.