/*
    ...
    property to all user-defined views in the specified database that do not have a
    description yet.
    If all user-defined views already have an MS_Description extended property, NULL is returned.
...
FROM
	[sys].[views] [v]
LEFT JOIN
	[sys].[extended_properties] [ep] ON [v].[object_id] = [ep].[major_id] AND [ep].[name] = N'MS_Description' AND [ep].[minor_id] = 0
WHERE
	[v].[is_ms_shipped] = 0  -- Excluding system views.
AND
	[ep].[value] IS NULL;  -- Excluding views that already have a description.