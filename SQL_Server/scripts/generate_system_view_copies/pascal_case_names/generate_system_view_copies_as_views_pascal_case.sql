/*
	...
	[sys] and [INFORMATION_SCHEMA] schemas of the specified database.

	The generated view names and column names are converted to PascalCase.
...
ELSE BEGIN
	...
	/* 
		...
		views previously created by this script in the specified schemas whose names
		...
		Only views with names matching the generated PascalCase system view names
		are dropped. Other user-defined views in the specified schemas remain
		unaffected.
	*/
	...
	SET @DropViews = (
		...
		AND
		-- Only drop views whose names match the generated PascalCase names of the
		-- original system views. This ensures that if an existing schema is used
		-- instead of a dedicated schema, only views corresponding to the original
		-- system views are dropped, while other user-defined views in the schema
		-- remain unaffected.
			[v].[name] IN (
				SELECT DISTINCT
					[ViewNamePascalCase]
	...
	WITH [cteCreateViewStatements] AS (
		SELECT
			...
			N'-- =============================================' + CHAR(13) + CHAR(10) + 
			N'DROP VIEW IF EXISTS ' + CASE WHEN SCHEMA_NAME([av].[schema_id]) = N'sys' THEN QUOTENAME(@SchemaForSys) WHEN SCHEMA_NAME([av].[schema_id]) = N'INFORMATION_SCHEMA' THEN QUOTENAME(@SchemaForInformationSchema) ELSE NULL END + N'.' + QUOTENAME([#MPC].[ViewNamePascalCase]) + N';' + CHAR(13) + CHAR(10) +
			N'GO' + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10) +
			N'CREATE VIEW ' + CASE WHEN SCHEMA_NAME([av].[schema_id]) = N'sys' THEN QUOTENAME(@SchemaForSys) WHEN SCHEMA_NAME([av].[schema_id]) = N'INFORMATION_SCHEMA' THEN QUOTENAME(@SchemaForInformationSchema) ELSE NULL END + N'.' + QUOTENAME([#MPC].[ViewNamePascalCase]) + CHAR(13) + CHAR(10) +   
