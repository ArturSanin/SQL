...
-- Description:	This stored procedure refreshes the table [DbInfo].[StoredProcedures]
--				by updating the metadata overview of all stored procedures in the database
--				(T-SQL, CLR, and extended).
-- =============================================
CREATE PROCEDURE [DbInfo].[uspRefreshStoredProcedures] 
AS
BEGIN

	SET NOCOUNT ON;

    TRUNCATE TABLE [DbInfo].[StoredProcedures];
...
-- uspRefreshStoredProcedures
-- =======================================

-- Stored procedure description.
EXEC [sys].[sp_addextendedproperty]
    @name = N'MS_Description',
    @value = N'This stored procedure refreshes the table [DbInfo].[StoredProcedures] by updating the metadata overview of all stored procedures in the database (T-SQL, CLR, and extended).',
    @level0type = N'SCHEMA', @level0name = N'DbInfo',
    @level1type = N'PROCEDURE', @level1name = N'uspRefreshStoredProcedures';
GO