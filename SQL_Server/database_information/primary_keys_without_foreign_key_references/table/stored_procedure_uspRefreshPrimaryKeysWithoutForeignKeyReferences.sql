...
-- Description:	This stored procedure refreshes the table
--				[DbInfo].[PrimaryKeysWithoutForeignKeyReferences] by updating it with
--				all primary keys in the database that are not referenced by any foreign key.
-- =============================================
CREATE PROCEDURE [DbInfo].[uspRefreshPrimaryKeysWithoutForeignKeyReferences] 
AS
BEGIN

	SET NOCOUNT ON;

    TRUNCATE TABLE [DbInfo].[PrimaryKeysWithoutForeignKeyReferences];
	...
	INSERT INTO
		[DbInfo].[PrimaryKeysWithoutForeignKeyReferences]
	SELECT
...
-- uspRefreshPrimaryKeysWithoutForeignKeyReferences
-- =======================================

-- Stored procedure description.
EXEC [sys].[sp_addextendedproperty]
    @name = N'MS_Description',
    @value = N'This stored procedure refreshes the table [DbInfo].[PrimaryKeysWithoutForeignKeyReferences] by updating it with all primary keys in the database that are not referenced by any foreign key.',
    @level0type = N'SCHEMA', @level0name = N'DbInfo',
    @level1type = N'PROCEDURE', @level1name = N'uspRefreshPrimaryKeysWithoutForeignKeyReferences';
GO